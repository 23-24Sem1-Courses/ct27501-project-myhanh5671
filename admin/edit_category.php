...
<head>
    ...
    <title>Chỉnh sửa danh mục sách</title>
...
<body>
    ...
    <main class="container" style="margin-top: 50px; margin-bottom: 100px">
        <div class="row">
            ...
            <p class="text-center">Chỉnh sửa danh mục sách</p>
        </div>
        <div class="row mb-2">
        ...
        <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
            <div class="mb-3">
                <label class="form-label">Tên loại sách hiện tại:</label>
                <input type="text" value="<?php if (isset($current_category_name)) {echo $current_category_name;}?>"
                    class="form-control" readonly>
                <label for="name_category_edit" class="form-label mt-2">Nhập lại tên mới cho loại sách:</label>
                <input type="text" name="name_category_edit" class="form-control" id="name_category_edit">

                <input type="hidden" name="id_of_category_edit_category"
                    value="<?php if (isset($_GET['id_of_category_edit_category'])) {echo $_GET['id_of_category_edit_category'];}?>">
            </div>
            <button type="submit" id="btn_edit_category_name" name="btn_edit_category_name">Cập nhật</button>
        </form>
    </main>
    <!-- footer dùng chung -->
    <footer>
